...
const { sshUser, sshServer, sshPort, secrets } = require('../config/config.json');
...
module.exports = {
  ...
  start: function(options = {}) {
    ...
    if (this.started) {
      ...
      if (secrets && helljs.test('-f', `${homedir}/.secrets/${secrets}`)) {
        shelljs.exec(`pbcopy < ${homedir}/.secrets/${secrets}`);
      }
